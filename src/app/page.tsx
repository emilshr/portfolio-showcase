...
import { EditYourSubmission } from "@src/components/EditYourSubmission";

export default async function Home() {
  const data = await api.portfolio.getHighlightOfTheDay.query();

  return (
    <main className="flex min-h-screen flex-col bg-black px-2 pb-4 text-slate-100 sm:px-4">
      <div className="flex w-full flex-col gap-y-4 pt-40 sm:pt-72">
        <div className="flex h-full w-full flex-col items-center gap-y-4">
          ...
          {data ? <EditYourSubmission item={data} /> : <AddYourPortfolio />}
